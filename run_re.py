...
def trainamp(args, model, tokenizer):
    """ Train the model """
    ...
    dtype_size = 2 if args.fp16 else 4  # 根据训练精度调整
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                if args.max_grad_norm > 0:
                    # Unscales the gradients before clipping
...
def main():
    ...
    ##################################################################################################
    # select-cuda
    parser.add_argument("--cuda_device", default="1", type=str)  # "0"(single-gpu),           !!!! "0123"(multi-gpu)
    ##################################################################################################
    # select-train/test
